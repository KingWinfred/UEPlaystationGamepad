...
void FRawWindowsDeviceEntry::InitializeNameArrays()
{
	...
	PlaystationGamepad::DPadMap[0] = FGamepadKeyNames::DPadUp;
	PlaystationGamepad::DPadMap[2] = FGamepadKeyNames::DPadRight;
	PlaystationGamepad::DPadMap[4] = FGamepadKeyNames::DPadDown;
	PlaystationGamepad::DPadMap[6] = FGamepadKeyNames::DPadLeft;

	PlaystationGamepad::psID[0] = { 1356, 1476, 4 }; // DS4 GEN1
	PlaystationGamepad::psID[1] = { 1356, 2508, 4 }; // DS4 GEN2
	PlaystationGamepad::psID[2] = { 1356, 3302, 7 }; // DualSense
    PlaystationGamepad::psID[3] = { 1356, 616,  0 }; // DualShock3
}
...
FString FRawInputWindows::GetDeviceNameByDeviceConfig(const FConnectedDeviceInfo* ConnectedDevice)
{

	int32 VID = ConnectedDevice->RIDDeviceInfo.hid.dwVendorId;
	int32 PID = ConnectedDevice->RIDDeviceInfo.hid.dwProductId;
	...
	for (const FRawInputDeviceConfiguration& DeviceConfig : RawInputSettings->DeviceConfigurations)
	{
		...
		if (VID==VendorID && PID==ProductID && DeviceConfig.bOverrideHardwareDeviceIdString)
		{
			return DeviceConfig.OverriddenHardwareDeviceId;
		}
	}
...
void FRawInputWindows::CopyConnectedDeviceInfo(FRawWindowsDeviceEntry& RegisteredDevice, const FConnectedDeviceInfo* ConnectedDevice)
{
	if (ConnectedDevice)
	{
		...
		RegisteredDevice.DeviceData.DeviceName = GetDeviceNameByDeviceConfig(ConnectedDevice);
...
void FRawInputWindows::SendControllerEvents()
{
	...
	for (TPair<int32, FRawWindowsDeviceEntry>& DeviceEntryPair : RegisteredDeviceList) {
		...
		if (DeviceEntry.bNeedsUpdate) {
			...
			for (PlaystationGamepad::PlaystationID& ps : PlaystationGamepad::psID) {
				if (DeviceEntry.DeviceData.VendorID == ps.vID && DeviceEntry.DeviceData.ProductID == ps.pID) {
					...
					if (!bIsRepeat) {
						if (iPrevValue != 8) {
							if (iPrevValue % 2 == 1) {
								MessageHandler->OnControllerButtonReleased(PlaystationGamepad::DPadMap[iPrevValue - 1], UserId, DeviceId, bIsRepeat);
								iPrevValue++;
								if (iPrevValue == 8) iPrevValue = 0;
							}
							MessageHandler->OnControllerButtonReleased(PlaystationGamepad::DPadMap[iPrevValue], UserId, DeviceId, bIsRepeat);
						}

						if (iValue != 8) {
							if (iValue % 2 == 1) {
								MessageHandler->OnControllerButtonPressed(PlaystationGamepad::DPadMap[iValue - 1], UserId, DeviceId, bIsRepeat);
								iValue++;
								if (iValue == 8) iValue = 0;
							}
							MessageHandler->OnControllerButtonPressed(PlaystationGamepad::DPadMap[iValue], UserId, DeviceId, bIsRepeat);
						}
